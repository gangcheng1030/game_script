...
type ChaoJiDou interface {
	...
	IsOnline(isLeader bool) bool
	QuitRole() // 返回到角色选择界面
	Quit()     // 退出游戏
	ForceQuit(isLeader bool)
...
func (c *chaoJiDou) SelectRole(n int, first bool, fullScreenMode int) {
	if first {
		...
		// 有未完成的任务，从主城开始
		robotgo.MoveSmooth(1013, 580, mouseSpeedX, mouseSpeedY)
		...
		// 调整移动方式
		robotgo.MoveSmooth(621+c.GameWindow.X, 134+c.GameWindow.Y, mouseSpeedX, mouseSpeedY)
		robotgo.Click()
		robotgo.Sleep(2)
		robotgo.MoveSmooth(658+c.GameWindow.X, 225+c.GameWindow.Y, mouseSpeedX, mouseSpeedY)
		robotgo.Click()
		robotgo.Sleep(2)
		robotgo.MoveSmooth(679+c.GameWindow.X, 565+c.GameWindow.Y, mouseSpeedX, mouseSpeedY)
		robotgo.Click()
		robotgo.Sleep(2)
...
func (c *chaoJiDou) isOnlineHelper() bool {
	...
	for _, pss := range pses {
		name, _ := pss.Name()
		if strings.HasPrefix(name, "NightmareBreaker.exe") {
			isOnline = true
			break
		}
	}
...
func (c *chaoJiDou) Quit() {
	...
	for _, pss := range pses {
		...
		if strings.HasPrefix(name, "LootHoarder.exe") || strings.HasPrefix(name, "NightmareBreaker.exe") ||
			strings.HasPrefix(name, "SystemSettings.exe") || strings.HasPrefix(name, "msedge.exe") ||
			strings.HasPrefix(name, CHROME_PROCESS_NAME) || strings.HasPrefix(name, "liebao.exe") ||
			strings.HasPrefix(name, T360_PROCESS_NAME) {
			pss.Terminate()
		}
	}
	robotgo.Sleep(15)
...
func (c *chaoJiDou) ForceQuit(isLeader bool) {
	var wg sync.WaitGroup
	if isLeader && len(Follwers) > 0 {
		for _, addr := range Follwers {
			tmpAddr := addr
...
func (c *chaoJiDou) forceQuitHelper() {
	// 退出启动器
	pses, _ := process.Processes()
	for _, pss := range pses {
		name, _ := pss.Name()
...
func (c *chaoJiDou) JiZhanYanSuan(dt int) {
	c.press(robotgo.KeyM, 2)
...
func (c *chaoJiDou) MiGong(dt int) {
	evChan := hook.Start()
	defer hook.End()
	for ev := range evChan {
		if ev.Kind == hook.KeyDown {
			...
			c.handleFollowersPressWithEvent(&ev, 0, 0)
...
func (c *chaoJiDou) handleFollowersPress(key string, st int) {
	...
	for _, addr := range Follwers {
		...
		go func() {
			...
			st1 := rand.Intn(5000)
			robotgo.MilliSleep(st1)
			rawCode, ok := utils.Keytoraw[key]
			if !ok {
				log.Printf("wrong key: %s\n", key)
				return
			}
			e := &hook.Event{
				Kind:    hook.KeyDown,
...
func (c *chaoJiDou) handleFollowersPressWithEvent(e *hook.Event, st int, st1 int) {
	if len(Follwers) == 0 {
		return
	}
